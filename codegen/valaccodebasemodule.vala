	...
	public void check_type (DataType type) {
		...
		if (array_type != null) {
			...
			if (array_type.element_type is ArrayType) {
				Report.error (type.source_reference, _("Stacked arrays are not supported"));
			} else if (array_type.element_type is DelegateType) {
				var delegate_type = (DelegateType) array_type.element_type;
				if (delegate_type.delegate_symbol.has_target) {
					Report.error (type.source_reference, _("Delegates with target are not supported as array element type"));
				}
			}
		}
