...
public class Vala.Parser : CodeVisitor {
	...
	void parse_declaration (Symbol parent, bool root = false) throws ParseError {
		...
		case TokenType.NEW:
			...
			if (!root) {
				throw new ParseError.SYNTAX (_("statements outside blocks allowed only in root namespace"));
			}
			rollback (begin);
	...
	void parse_method_declaration (Symbol parent, List<Attribute>? attrs) throws ParseError {
		...
		if (method.binding == MemberBinding.INSTANCE) {
			...
			if (ModifierFlags.ABSTRACT in flags
			    || ModifierFlags.VIRTUAL in flags
			    || ModifierFlags.OVERRIDE in flags) {
				throw new ParseError.SYNTAX (_("the modifiers `abstract', `virtual', and `override' are not valid for %s methods"), (ModifierFlags.CLASS in flags) ? "class" : "static");
			}
		}
	...
	void parse_property_declaration (Symbol parent, List<Attribute>? attrs) throws ParseError {
		...
		if (accept (TokenType.HASH)) {
			if (!context.deprecated) {
				Report.warning (get_last_src (), _("deprecated syntax, use `owned` modifier before `get'"));
			}
			getter_owned = true;
		}
		...
		while (current () != TokenType.CLOSE_BRACE) {
			if (accept (TokenType.DEFAULT)) {
				if (prop.initializer != null) {
					throw new ParseError.SYNTAX (_("property default value already defined"));
				}
				expect (TokenType.ASSIGN);
